...
import hospital.OAS.AddStaffOAS;
...
public class StaffController {
    ...
    @QueryParam("phoneNumber") String phoneNumber) {
        return StaffService.getfilter(maxResult,page,nama,email,phoneNumber);
    }
    
    @POST
    @Operation(summary = "Menanbahkan data staff")
    @RequestBody(content = @Content(mediaType = MediaType.APPLICATION_JSON,schema = @Schema(implementation = AddStaffOAS.Request.class)))
    @APIResponses( value = {
        @APIResponse(responseCode = "200", description = "OK",content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = AddStaffOAS.Response.class))),
    ...
    public Response update(JsonObject params, @Parameter(name = "id") @PathParam("id") Long id){
        return StaffService.update(params, id);
    }
