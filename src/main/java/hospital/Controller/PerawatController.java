...
import hospital.OAS.AddPerawatOAS;
import hospital.service.PerawatService;
...
public class PerawatController {
    ...
    @QueryParam("phoneNumber") String phoneNumber) {
        return perawatService.getfilter(maxResult,page,nama,email,phoneNumber);
    }
    
    @POST
    @Operation(summary = "Menanbahkan data perawat")
    @RequestBody(content = @Content(mediaType = MediaType.APPLICATION_JSON,schema = @Schema(implementation = AddPerawatOAS.Request.class)))
    @APIResponses( value = {
        @APIResponse(responseCode = "200", description = "OK",content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = AddPerawatOAS.Response.class))),
    ...
    public Response update(JsonObject params, @Parameter(name = "id") @PathParam("id") Long id){
       return perawatService.update(params, id);
    }
